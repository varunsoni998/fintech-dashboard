...
import {
  ...
  Check, Loader2, Building2, Key, Bell, Palette, ChevronRight,
} from "lucide-react";
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "8px 0 40px" }}>
        ...
        <Card style={{ marginBottom: 20 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ background: BG, boxShadow: SHADOW_IN, borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8 }}>
              <User style={{ width: 14, height: 14, color: TEXT_MUTED, flexShrink: 0 }} />
              <input
                value={fullName}
                onChange={(e) => setFullName(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && saveProfile()}
                placeholder="Your full name"
                style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontSize: 13, color: TEXT_MAIN, fontFamily: "inherit" }}
              />
            </div>
          </div>
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ background: BG, boxShadow: SHADOW_IN, borderRadius: 12, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, opacity: 0.6 }}>
              <Mail style={{ width: 14, height: 14, color: TEXT_MUTED, flexShrink: 0 }} />
              <span style={{ fontSize: 13, color: TEXT_MAIN }}>{displayEmail}</span>
            </div>
          </div>
          <button
            onClick={saveProfile}
        ...
        <Card style={{ marginBottom: 20 }}>
          ...
          <Row icon={dark ? Moon : Sun} label="Dark Mode" sublabel={dark ? "Currently on" : "Currently off"}
            color={dark ? "#7B8FE0" : "#E0AA5A"}
            right={<Toggle value={dark} onChange={toggleDark} dark={dark} />}
          />
          <Row icon={Bell} label="Notifications" sublabel={notifications ? "Enabled" : "Disabled"}
            color="#52A882"
            right={<Toggle value={notifications} onChange={() => setNotifications((n) => !n)} dark={dark} />}
          />
          <Row icon={Palette} label="Theme" sublabel="Neumorphic light / dark" color="#9B8FE8"
            right={<span style={{ fontSize: 11, color: TEXT_MUTED, fontWeight: 600 }}>{dark ? "Dark" : "Light"}</span>}
          />
        </Card>
        ...
        <Card style={{ marginBottom: 20 }}>
          ...
          <Row icon={Key} label="Change Password" sublabel="Send a reset link to your email" color="#E07A5A"
          ...
          />
          <Row icon={Shield} label="Role" sublabel="Contact admin to change your role" color="#6B7FD4"
            right={<span style={{ fontSize: 11, fontWeight: 700, color: "#7B8FE0", background: "rgba(123,143,224,0.12)", borderRadius: 99, padding: "3px 10px" }}>{displayRole}</span>}
          />
          <Row icon={Building2} label="Organisation ID" sublabel={profile?.org_id ?? "Not assigned"} color={TEXT_MUTED} right={<></>} />
        </Card>
